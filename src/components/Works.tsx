          ...
          {works.map((work) => (
            ...
            >
              <div className="relative h-64 overflow-hidden bg-stone-200">
                {work.images && work.images[0] && work.images[0].trim() !== '' ? (
                  <img 
                    src={work.images[0]} 
                    alt={work.title || 'Obra'} 
              ...
              <div className="p-6">
                <h3 className="text-xl font-bold text-emerald-900 mb-2">{work.title || 'Projeto Pindorama'}</h3>
                <p className="text-stone-600 line-clamp-2">{work.description || 'Confira detalhes desta obra realizada.'}</p>
        ...
        >
          ...
          <div className="relative w-full max-w-5xl aspect-video" onClick={e => e.stopPropagation()}>
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/50 px-4 py-2 rounded-full text-white text-sm">
                {currentImageIndex + 1} / {(currentWork.images?.length || 0)}
              </div>
            </div>
